...
const DocumentViewer = ({ documentUrl, documentTitle, fileType, className }) => {
  ...
  // Log props on initial render and updates
  console.log('📄 DocumentViewer Props:', { documentUrl, documentTitle, fileType, className });

  // Determine format name based on passed fileType
  const formatName = fileType ? fileType.toUpperCase() : 'File'; 

  useEffect(() => {
    console.log('📄 DocumentViewer Effect Trigger:', { documentUrl, fileType }); // Log inside effect
    if (!documentUrl) {
