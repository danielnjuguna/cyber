...
if (process.env.DB_SSL_REQUIRED === 'true') {
  ...
  if (!caPath) {
    ...
    // HARDCODED PATH - Replace dynamic resolution temporarily
    const fullCaPath = 'C:/Users/Administrator/OneDrive/Desktop/CyberDocs/certs/ca.pem'; 
    console.log(`Using HARDCODED CA certificate path: ${fullCaPath}`);
    // const caPath = process.env.DB_SSL_CA_PATH;
    // const fullCaPath = path.resolve(__dirname, '..', caPath);
    // console.log(`Resolved CA certificate path (relative method): ${fullCaPath}`);
    try {
